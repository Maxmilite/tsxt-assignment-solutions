...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl
    {
        ...
        )
        {
            ...
            // std::cerr << std::fixed << std::setprecision(5) << "Stage-1" << ": " << std::chrono::system_clock::now().time_since_epoch().count() * 1.0 * std::chrono::system_clock::period::num / std::chrono::system_clock::period::den << std::endl;
            // build our feature pyramid
            
            
            // Target: SLOW
            // Optimized at impl_extract_fhog_features in fhog.h
...
// ----------------------------------------------------------------------------------------
    ...
    )
    {
        unsigned long width, height;
        compute_fhog_window_size(width,height);
        // std::cerr << std::fixed << std::setprecision(5) << "Time-Load-1" << ": " << std::chrono::system_clock::now().time_since_epoch().count() * 1.0 * std::chrono::system_clock::period::num / std::chrono::system_clock::period::den << std::endl;
        impl::create_fhog_pyramid<Pyramid_type>(img, fe, feats, cell_size, height,
            ...
            max_pyramid_levels);
        // std::cerr << std::fixed << std::setprecision(5) << "Time-Load-2" << ": " << std::chrono::system_clock::now().time_since_epoch().count() * 1.0 * std::chrono::system_clock::period::num / std::chrono::system_clock::period::den << std::endl;
    }
...
// ----------------------------------------------------------------------------------------
    ...
    namespace impl
    {
        ...
        ) 
        {
            ...
            // std::cerr << "scan_fhog_pyramid called." << std::endl;
            ...
            for (unsigned long l = 0; l < feats.size(); ++l)
            {
                ...
                // Target: SLOW
                // To be checked
                // May be a O(n ^ 2) algorithm can be optimized with data parallel
